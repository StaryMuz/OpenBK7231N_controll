...
def je_cena_pod_limitem(df, predstih=False):
    ...
    row = df[
        df["Ctvrthodina"] == ctvrthodina_index
    ]

    if row.empty:
        raise Exception(
            f"Nenalezena cena pro periodu {ctvrthodina_index}."
        )
...
def rozhodni_spusteni_cyklu():
    """
    Vyhodnotí, zda je potřeba použít předstih.

    Předstih pouze při:
    poslední stav = OFF
    následující požadovaný stav = ON
    """
    ...
    pod_limitem, cena = je_cena_pod_limitem(
        df,
        predstih=True
    )
    ...
    print(
        f"Poslední stav: {posledni_stav}"
    )

    print(
        f"Požadovaný stav následující čtvrthodiny: "
        f"{pozadovany_stav}"
    )

    if (
        posledni_stav == 0
        and pozadovany_stav == 1
    ):
        print(
            "Detekován přechod OFF → ON, "
            "použije se předstih."
        )
        return True

    print(
        "Předstih není potřeba."
    )
...
if __name__ == "__main__":
    ...
    if now.minute <= DO_KDY_PLATI_NOVA_HODINA:

        print(
            "Workflow spuštěn v nové hodině – "
            "první cyklus se spustí ihned."
        )
        ...
        next_quarter = dalsi_ctvrthodina(now)

        rozhodovaci_cas = (
            next_quarter
            -
            timedelta(minutes=PREDSTIH_MINUT)
        )

        if now < rozhodovaci_cas:

            print(
                f"Čekám do rozhodovacího času "
                f"{rozhodovaci_cas.strftime('%H:%M:%S')}"
            )
            ...
            print(
                "Jsme mezi rozhodovacím časem "
                "a začátkem čtvrthodiny."
            )
    ...
    while True:
        ...
        print(
            f"Čekám na rozhodnutí před cyklem "
        ...
        if rozhodni_spusteni_cyklu():
            ...
            print(
                f"Čekám na začátek čtvrthodiny "
                f"{next_cycle.strftime('%H:%M:%S')}"
            )
